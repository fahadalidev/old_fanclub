...
class UpdatesController extends Controller
{
  ...
  public function image($id, $path)
    ...
    {
      $updates = Updates::where('user_id', '<>', auth()->user()->id)->whereLocked('no')->orderBy('id', 'desc')->paginate($this->settings->number_posts_show);
    ...
    public function ajaxExplore()
    {
      ...
      $data = Updates::where('user_id', '<>', auth()->user()->id)->whereLocked('no')->orderBy('id', 'desc')->skip($skip)->take($this->settings->number_posts_show)->get();
      $counterPosts = ($total - $this->settings->number_posts_show - $skip);
